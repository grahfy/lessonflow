...
- When using the `release-notes` skill, update every canonical version reference in the same task.
- At minimum, update the GitLab changelog, [`package.json`](/home/grahf/jon/melbourne-guitar-school/package.json), and any other repo-owned file that states the shipped application version.
- Treat version sync as part of the release change itself, not a follow-up cleanup step.
