...
if ! grep -Fq "ProtectSystem=false" "${COMPAT_FILE}" || ! grep -Fq "PrivateTmp=false" "${COMPAT_FILE}" || ! grep -Fxq "ReadWritePaths=" "${COMPAT_FILE}"; then
...
if ! grep -Fq "Detected lessonflow.service namespace failure; installing" "${OUTPUT_LOG}"; then
  echo "Expected namespace compatibility warning"
  cat "${OUTPUT_LOG}"
  exit 1
fi
...
echo "Update app service namespace compatibility test passed."
