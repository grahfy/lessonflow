...
if ! grep -Fq "lessonflow.service status:" "${OUTPUT_LOG}"; then
  echo "Expected failure diagnostics to include systemctl status heading"
  cat "${OUTPUT_LOG}"
  exit 1
fi

if ! grep -Fq "Recent lessonflow.service logs:" "${OUTPUT_LOG}"; then
  echo "Expected failure diagnostics to include journalctl heading"
  cat "${OUTPUT_LOG}"
  exit 1
fi

if ! grep -Fq "Effective lessonflow.service unit:" "${OUTPUT_LOG}"; then
  echo "Expected failure diagnostics to include systemctl cat heading"
  cat "${OUTPUT_LOG}"
  exit 1
fi

if ! grep -Fq "status lessonflow --no-pager" "${SYSTEMCTL_LOG}"; then
  echo "Expected systemctl status to run on restart failure"
  cat "${OUTPUT_LOG}"
  cat "${SYSTEMCTL_LOG}"
  exit 1
fi

if ! grep -Fq "cat lessonflow" "${SYSTEMCTL_LOG}"; then
  echo "Expected systemctl cat to run on restart failure"
  cat "${OUTPUT_LOG}"
  cat "${SYSTEMCTL_LOG}"
  exit 1
fi

echo "Update app service failure diagnostics test passed."
