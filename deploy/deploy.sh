...
try_auto_repair_app_service_namespace_failure() {
    ...
    if [[ -f "${override_file}" ]]; then
        ...
        run_sudo_cmd systemctl reset-failed "${APP_NAME}" >/dev/null 2>&1 || true
...
start_spinner() {
    ...
    (
        while true; do
            ...
            printf "\r\033[K${CYAN}%s${NC} %s ${DIM}%s${NC}" "${SPINNER_FRAMES[$i]}" "${SPINNER_MSG}" "${suffix}"
            i=$(( (i + 1) % frame_count ))
            sleep 0.2
        done
    ) &
...
remove_legacy_cron_entries() {
    ...
    if run_sudo_cmd crontab "${tmp_file}" 2>/dev/null; then
        log_info "Removed legacy managed cron entries (migrated to systemd timers)"
    else
